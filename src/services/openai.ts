  ...
  "exercises": [
    {
      ...
      "intervalCount": 4,
      // rounds: how many times the athlete gets through the WHOLE rotation — the board's own
      // "(4 rounds)". On the EMOM above that is 4, NOT 16: sixteen windows over four stations is
      // four rounds. rounds x stations = intervalCount. Never copy the window count in here.
      "rounds": 4,
...
## KEY GUIDELINES
1. Only split into multiple exercises for truly separate PRACTICES (e.g., Strength + Metcon, Skill + WOD). A single WOD = one exercise, ALWAYS. A piece made of several movement blocks under ONE governing clock/header — "[10:00 AMRAP, 2:00 REST] x 3 rounds:" over blocks A/B/C, "A.1/A.2", numbered lists 1./2./3. — is STILL ONE exercise: emit one "sections" entry per block, each with its own "label" and its own "scoreType" so each block keeps its OWN score. A section's "scoreType" says the athlete EARNS a number in that block; a block whose every rep is written on the board earns nothing and must not carry one.
NOT THIS: an EMOM whose minutes hold different movements ("min 1: 8-10 Deadlift / min 2: 12-15 Push-up / min 3: 15 Box Jump"). That is a STATION ROTATION, not a set of scored blocks — the reps are all prescribed and the athlete earns nothing per minute. Use flat "movements[]" with "stationLabel" ("Min 1", "Min 2"…) and "countingMode": "per_station_visit", exactly as ROTATING STATION LABELS says. NO sections, NO per-minute "scoreType". An EMOM repeating the SAME movements every minute is the same shape with one station: still flat movements[], still no sections. The number of stations is not a reason to structure the board differently. NEVER split those blocks into separate exercises. A separate exercise means a separate piece of training with its own poster; blocks are parts of one piece and must share one. SEQUENTIAL LIFTS UNDER ONE HEADING = ONE exercise with SECTIONS (not separate exercises, not one flat block): when a strength piece flows one movement "Into:"/"then" another under a single heading and EACH block has its OWN set count — e.g. "4 sets Every 1:30: 2 Push Press / Into: / 4 sets Every 1:30: 2 Push Jerk" — emit ONE exercise whose "sections" has one "rounds" section PER block, each with that block's own "rounds" (set count) and its own movement(s). Also list each movement once in top-level "movements[]" for reference. These blocks are done SEQUENTIALLY at INDEPENDENT weights (Push Press across its 4 sets, THEN Push Jerk across its 4 sets), so they must NOT be flattened into one shared block. This differs from a SIMULTANEOUS "+"-joined complex done together each set on one bar (e.g. "1 Power Clean + 1 Push Jerk"), which stays a single flat "movements[]" list with NO sections (one shared weight). See examples 4b (sequential → sections) and 4c (simultaneous → flat).
2. Exercise names MUST include set count/timing (e.g., "8 Rounds For Time", "5 sets every 2:30"). "AxB" = A sets of B reps.
3. Movement alternatives ("40 DU / 60 singles", "10 Pull-up / Ring Row"): the harder (Rx) movement is the primary and the scaled option goes in "alternative", whichever side the board writes first. Do NOT create two separate movements.
...
Output:
{
  ...
  "exercises": [{ "name": "Partner RFT (6 each)", "type": "wod", "loggingMode": "for_time", "prescription": "6 rounds each: 10 DL 60/40kg, 40 DU/60 SU, 15 Box Jumps", "suggestedSets": 6,
    ...
    "movements": [
      ...
      { "name": "Double Under", "reps": 40, "inputType": "none", "alternative": { "name": "Single Under", "reps": 60 } },
