  ...
  return (
    <div style={{ width: '100%', position: 'relative', transform: 'rotate(-1.3deg)' }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: fD, fontSize: named ? 28 : 32, fontWeight: 900, lineHeight: 1, marginTop: 8, color: BRAND.paperInk, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          <span style={named ? hl() : undefined}>{named ? wod.title : wod.format}</span>
        </div>
        <div style={{ fontFamily: fH, fontSize: 22, color: '#5a4628', marginTop: 3 }}>
          ...
          {rows.map((r, i) =>
            r.kind === 'block' ? (
              <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: i ? 10 : 0 }}>
                {r.label && <span style={{ fontFamily: fD, fontSize: 16, fontWeight: 900, letterSpacing: '0.03em' }}>{r.label}</span>}
                {r.cap && <span style={{ fontFamily: fD, fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', color: '#7a6038', textTransform: 'uppercase' }}>{r.cap}</span>}
                <span style={{ flex: 1 }} />
                {r.score && (
                ...
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr max-content', alignItems: 'center', gap: 16, padding: '1.5px 0' }}>
                  ...
                  {parts.isStrength ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', lineHeight: 1.1 }}>
                      <span style={{ fontFamily: fH, fontSize: 23, fontWeight: 700, color: BRAND.paperInk, transform: 'rotate(-2deg)', display: 'inline-block', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 8 }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontFamily: fH, fontSize: 18, color: '#5a4628', lineHeight: 1 }}>{wod.result.label.toLowerCase()}</div>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, flexWrap: 'nowrap', marginTop: 2 }}>
              <span style={{ fontFamily: fD, fontSize: 80, fontWeight: 900, lineHeight: 0.88, color: BRAND.paperInk, whiteSpace: 'nowrap' }}>
                <span style={hl()}>{wod.result.value.split(' ')[0]}</span>
              </span>
              {wod.result.value.includes(' ') && (
                <span style={{ fontFamily: fD, fontSize: 28, fontWeight: 700, color: '#7a6038', whiteSpace: 'nowrap', paddingBottom: 6 }}>
                  {wod.result.value.split(' ').slice(1).join(' ')}
                </span>
              )}
            </div>
          </div>
          <VibeStamp vibe={vibe} color={BRAND.paperInk} scale={0.68} />
        </div>
