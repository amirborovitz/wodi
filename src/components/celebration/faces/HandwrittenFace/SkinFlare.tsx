    ...
    <div style={{
      ...
      <div style={{ position: 'relative', padding: '20px 20px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <HeaderMeta
            effort={<EffortMeta ep={wod.ep} color="rgba(0,0,0,0.86)" />}
            date={<PosterDate date={wod.date} surface="light" style={{ fontFamily: fM, fontSize: 10, color: 'rgba(0,0,0,0.55)', letterSpacing: '0.04em' }} />}
          />
        </div>
          ...
          {rows.map((r, i) =>
            r.kind === 'block' ? (
              <div key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: i ? 12 : 0, marginBottom: 2 }}>
                ...
                <BlockHeaderRule ruled={r.ruled} color="rgba(0,0,0,0.18)" />
                {r.score && (
              ...
              return (
                <React.Fragment key={i}>
                  <div style={stationRowChrome({ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) max-content', alignItems: 'center', gap: 16, padding: '3px 0', borderBottom: '1px solid rgba(0,0,0,0.08)' }, parts.isStation, null)}>
                    {parts.roundLabel ? (
                    ...
                    {parts.isStrength ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', lineHeight: 1.05 }}>
                        <span style={parts.teamVoice === 'computed' ? computedVoice(BRAND.ink) : { fontFamily: fH, fontSize: 20, fontWeight: 700, color: BRAND.ink, transform: 'rotate(-2deg)', display: 'inline-block', whiteSpace: 'nowrap' }}>
                          {parts.team}
                        </span>
                        {parts.me && (
                          <span style={parts.meIsLoad ? loadVoice('rgba(0,0,0,0.5)') : { fontFamily: fB, fontSize: 13, fontWeight: 800, color: 'rgba(0,0,0,0.75)', whiteSpace: 'nowrap' }}>
                            {parts.me}
                          </span>
                        )}
                      </div>
                    ) : parts.single ? (
                      <span style={parts.singleVoice === 'load' ? loadVoice('rgba(0,0,0,0.5)')
                        : parts.singleVoice === 'computed' ? computedVoice(BRAND.ink)
                        : { fontFamily: fH, fontSize: 20, fontWeight: 700, color: BRAND.ink, transform: 'rotate(-2deg)', display: 'inline-block', whiteSpace: 'nowrap' }}>
                        {parts.single}
                      </span>
                    ) : <span />}
                  </div>
        ...
        <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 8 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <ResultValue
              ...
              scoreLabelStyle={{ color: 'rgba(0,0,0,0.55)' }}
              scoreDividerColor="rgba(0,0,0,0.28)"
